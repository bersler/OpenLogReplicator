...
namespace OpenLogReplicator {
    ...
    bool DbTable::matchesCondition(const Ctx* ctx, char op, const std::unordered_map<std::string, std::string>* attributes) {
        ...
        if (unlikely(ctx->trace & Ctx::TRACE::CONDITION))
            ctx->logTrace(Ctx::TRACE::CONDITION, "matchesCondition: table: " + owner + "." + name + ", condition: " + conditionStr + ", result: " +
                                                 std::to_string(result));
        return result;
    }

    void DbTable::setConditionStr(const std::string& newConditionStr) {
