...
namespace OpenLogReplicator {
    ...
    void TransactionBuffer::addTransactionChunk(Transaction* transaction, RedoLogRecord* redoLogRecord) {
        ...
        if (unlikely(transaction->lastSplit)) {
            ...
            auto lastTc = reinterpret_cast<TransactionChunk*>(ctx->swappedMemoryLast(transaction->xid));
        ...
        // New block
        if (ctx->swappedMemorySize(transaction->xid) == 0 ||
                reinterpret_cast<TransactionChunk*>(ctx->swappedMemoryLast(transaction->xid))->size + chunkSize > TransactionChunk::DATA_BUFFER_SIZE)
            ctx->swappedMemoryGrow(transaction->xid);
    ...
    void TransactionBuffer::rollbackTransactionChunk(Transaction* transaction) {
        ...
        ctx->swappedMemoryShrink(transaction->xid);
    }
