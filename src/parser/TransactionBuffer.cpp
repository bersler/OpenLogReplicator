...
namespace OpenLogReplicator {
    ...
    void TransactionBuffer::addTransactionChunk(Transaction* transaction, RedoLogRecord* redoLogRecord) {
        typeChunkSize chunkSize = redoLogRecord->size + ROW_HEADER_TOTAL;
        ...
        if (unlikely(transaction->lastSplit)) {
            ...
            TransactionChunk* lastTc = transaction->memoryChunks.last();
            typeChunkSize lastSize = *reinterpret_cast<typeChunkSize*>(lastTc->buffer + lastTc->size - sizeof(typeChunkSize));
            RedoLogRecord* last501 = reinterpret_cast<RedoLogRecord*>(lastTc->buffer + lastTc->size - lastSize + ROW_HEADER_DATA0);
        ...
        // New block
        if (transaction->memoryChunks.size() == 0 || transaction->memoryChunks.last()->size + chunkSize > TransactionChunk::DATA_BUFFER_SIZE)
            transaction->memoryChunks.grow(ctx);

        // Append to the chunk at the end
        TransactionChunk* lastTc = transaction->memoryChunks.last();
        *reinterpret_cast<typeOp2*>(lastTc->buffer + lastTc->size + ROW_HEADER_OP) = redoLogRecord->opCode << 16;
        RedoLogRecord* redoLogRecordTarget1 = reinterpret_cast<RedoLogRecord*>(lastTc->buffer + lastTc->size + ROW_HEADER_DATA0);
    ...
    void TransactionBuffer::addTransactionChunk(Transaction* transaction, RedoLogRecord* redoLogRecord1, const RedoLogRecord* redoLogRecord2) {
        ...
        *reinterpret_cast<typeOp2*>(lastTc->buffer + lastTc->size + ROW_HEADER_OP) = (redoLogRecord1->opCode << 16) | redoLogRecord2->opCode;
        ...
        RedoLogRecord* redoLogRecordTarget2 = reinterpret_cast<RedoLogRecord*>(lastTc->buffer + lastTc->size + ROW_HEADER_DATA1 + redoLogRecord1->size);
    ...
    void TransactionBuffer::rollbackTransactionChunk(Transaction* transaction) {
        ...
        typeChunkSize chunkSize = *reinterpret_cast<typeChunkSize*>(lastTc->buffer + lastTc->size - sizeof(typeChunkSize));
        lastTc->size -= chunkSize;
        --lastTc->elements;
        transaction->size -= chunkSize;
        ...
        transaction->memoryChunks.shrink(ctx);
    }
    ...
    uint8_t* TransactionBuffer::allocateLob(const RedoLogRecord* redoLogRecord1) const{
        ...
        *reinterpret_cast<typeTransactionSize*>(data) = lobSize;
        ...
        RedoLogRecord* redoLogRecord1new = reinterpret_cast<RedoLogRecord*>(data + sizeof(typeTransactionSize));
