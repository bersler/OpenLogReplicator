...
namespace OpenLogReplicator {
    ...
    template<class VALUE, SysCol::COLTYPE COLTYPE> void SystemTransaction::updateValue(VALUE& val, typeCol column, const DbTable* table, uint64_t offset,
        ...
        if (builder->values[column][static_cast<uint>(Builder::VALUE_TYPE::AFTER)] != nullptr &&
            builder->sizes[column][static_cast<uint>(Builder::VALUE_TYPE::AFTER)] > 0) {
            char* retPtr;
            ...
            if constexpr (COLTYPE == SysCol::COLTYPE::NUMBER) {
                builder->parseNumber(builder->values[column][static_cast<uint>(Builder::VALUE_TYPE::AFTER)],
                                     builder->sizes[column][static_cast<uint>(Builder::VALUE_TYPE::AFTER)], offset);
                ...
                builder->parseRaw(builder->values[column][static_cast<uint>(Builder::VALUE_TYPE::AFTER)],
                                  builder->sizes[column][static_cast<uint>(Builder::VALUE_TYPE::AFTER)], offset);
                ...
                builder->parseString(builder->values[column][static_cast<uint>(Builder::VALUE_TYPE::AFTER)],
                                     builder->sizes[column][static_cast<uint>(Builder::VALUE_TYPE::AFTER)],
        ...
        } else if (builder->values[column][static_cast<uint>(Builder::VALUE_TYPE::AFTER)] != nullptr ||
                   builder->values[column][static_cast<uint>(Builder::VALUE_TYPE::BEFORE)] != nullptr) {
            if (unlikely(ctx->isTraceSet(Ctx::TRACE::SYSTEM))) {
                if constexpr (std::is_same<VALUE, typeIntX>::value)
    ...
    template<class TABLE> void SystemTransaction::updateValues(const DbTable* table, TABLE* row, typeCol column, uint64_t offset) {
        if constexpr (std::is_same<TABLE, SysCCol>::value) {
            if (table->columns[column]->name == "CON#") {
                ...
                updateValue<typeCol, SysCol::COLTYPE::NUMBER>(row->intCol, column, table, offset);
                ...
                updateValue<typeIntX, SysCol::COLTYPE::NUMBER>(row->spare1, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, SysCol>::value) {
            if (table->columns[column]->name == "OBJ#") {
                ...
                updateValue<typeCol, SysCol::COLTYPE::NUMBER>(row->col, column, table, offset);
            } else if (table->columns[column]->name == "SEGCOL#") {
                updateValue<typeCol, SysCol::COLTYPE::NUMBER>(row->segCol, column, table, offset);
            } else if (table->columns[column]->name == "INTCOL#") {
                updateValue<typeCol, SysCol::COLTYPE::NUMBER>(row->intCol, column, table, offset);
                ...
                updateValue<uint, SysCol::COLTYPE::NUMBER>(row->length, column, table, offset);
            } else if (table->columns[column]->name == "PRECISION#") {
                updateValue<int, SysCol::COLTYPE::NUMBER>(row->precision, column, table, offset, -1);
            } else if (table->columns[column]->name == "SCALE") {
                updateValue<int, SysCol::COLTYPE::NUMBER>(row->scale, column, table, offset, -1);
            } else if (table->columns[column]->name == "CHARSETFORM") {
                updateValue<uint, SysCol::COLTYPE::NUMBER>(row->charsetForm, column, table, offset);
            } else if (table->columns[column]->name == "CHARSETID") {
                updateValue<uint, SysCol::COLTYPE::NUMBER>(row->charsetId, column, table, offset);
            } else if (table->columns[column]->name == "NULL$") {
                updateValue<int, SysCol::COLTYPE::NUMBER>(row->null_, column, table, offset);
            } else if (table->columns[column]->name == "PROPERTY") {
                updateValue<typeIntX, SysCol::COLTYPE::NUMBER>(row->property, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, SysDeferredStg>::value) {
            if (table->columns[column]->name == "OBJ#") {
                ...
                updateValue<typeIntX, SysCol::COLTYPE::NUMBER>(row->flagsStg, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, SysECol>::value) {
            if (table->columns[column]->name == "TABOBJ#") {
                ...
                updateValue<typeCol, SysCol::COLTYPE::NUMBER>(row->colNum, column, table, offset);
            } else if (table->columns[column]->name == "GUARD_ID") {
                updateValue<typeCol, SysCol::COLTYPE::NUMBER>(row->guardId, column, table, offset, -1);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, SysLob>::value) {
            if (table->columns[column]->name == "OBJ#") {
                ...
                updateValue<typeObj, SysCol::COLTYPE::NUMBER>(row->lObj, column, table, offset);
        ...
        if constexpr (std::is_same<TABLE, SysLobFrag>::value) {
            if (table->columns[column]->name == "FRAGOBJ#") {
                ...
                updateValue<typeObj, SysCol::COLTYPE::NUMBER>(row->parentObj, column, table, offset);
            } else if (table->columns[column]->name == "TS#") {
                updateValue<typeTs, SysCol::COLTYPE::NUMBER>(row->ts, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, SysObj>::value) {
            if (table->columns[column]->name == "OWNER#") {
                ...
                updateValue<typeObj, SysCol::COLTYPE::NUMBER>(row->dataObj, column, table, offset);
            } else if (table->columns[column]->name == "NAME") {
                ...
                updateValue<typeIntX, SysCol::COLTYPE::NUMBER>(row->flags, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, SysTab>::value) {
            if (table->columns[column]->name == "OBJ#") {
                ...
                updateValue<typeCol, SysCol::COLTYPE::NUMBER> (row->cluCols, column, table, offset);
            } else if (table->columns[column]->name == "FLAGS") {
                updateValue<typeIntX, SysCol::COLTYPE::NUMBER>(row->flags, column, table, offset);
            } else if (table->columns[column]->name == "PROPERTY") {
                updateValue<typeIntX, SysCol::COLTYPE::NUMBER>(row->property, column, table, offset);
        ...
        if constexpr (std::is_same<TABLE, SysTabComPart>::value) {
            if (table->columns[column]->name == "OBJ#") {
                ...
                updateValue<typeObj, SysCol::COLTYPE::NUMBER>(row->bo, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, SysTabSubPart>::value) {
            if (table->columns[column]->name == "OBJ#") {
                ...
                updateValue<typeObj, SysCol::COLTYPE::NUMBER>(row->pObj, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, SysTs>::value) {
            if (table->columns[column]->name == "TS#") {
                ...
                updateValue<std::string, SysCol::COLTYPE::VARCHAR>(row->name, column, table, offset, 0, SysTs::NAME_LENGTH);
            } else if (table->columns[column]->name == "BLOCKSIZE") {
                updateValue<uint32_t, SysCol::COLTYPE::NUMBER>(row->blockSize, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, SysUser>::value) {
            if (table->columns[column]->name == "USER#") {
                ...
                updateValue<std::string, SysCol::COLTYPE::VARCHAR>(row->name, column, table, offset, 0, SysUser::NAME_LENGTH);
            } else if (table->columns[column]->name == "SPARE1") {
                updateValue<typeIntX, SysCol::COLTYPE::NUMBER>(row->spare1, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, XdbTtSet>::value) {
            if (table->columns[column]->name == "GUID") {
                ...
                updateValue<std::string, SysCol::COLTYPE::VARCHAR>(row->tokSuf, column, table, offset, 0, XdbTtSet::TOKSUF_LENGTH);
            } else if (table->columns[column]->name == "FLAGS") {
                updateValue<uint64_t, SysCol::COLTYPE::NUMBER>(row->flags, column, table, offset);
            } else if (table->columns[column]->name == "OBJ#") {
                updateValue<typeObj, SysCol::COLTYPE::NUMBER>(row->obj, column, table, offset);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, XdbXNm>::value) {
            if (table->columns[column]->name == "NMSPCURI") {
                ...
                updateValue<std::string, SysCol::COLTYPE::RAW>(row->id, column, table, offset, 0, XdbXNm::ID_LENGTH);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, XdbXPt>::value) {
            if (table->columns[column]->name == "PATH") {
                ...
                updateValue<std::string, SysCol::COLTYPE::RAW>(row->id, column, table, offset, 0, XdbXPt::ID_LENGTH);
            }
        }
        ...
        if constexpr (std::is_same<TABLE, XdbXQn>::value) {
            if (table->columns[column]->name == "NMSPCID") {
                ...
                updateValue<std::string, SysCol::COLTYPE::VARCHAR>(row->localName, column, table, offset, 0, XdbXQn::LOCALNAME_LENGTH);
            } else if (table->columns[column]->name == "FLAGS") {
                updateValue<std::string, SysCol::COLTYPE::RAW>(row->flags, column, table, offset, 0, XdbXQn::FLAGS_LENGTH);
            } else if (table->columns[column]->name == "ID") {
                updateValue<std::string, SysCol::COLTYPE::RAW>(row->id, column, table, offset, 0, XdbXQn::ID_LENGTH);
            }
        }
    }
    ...
    void SystemTransaction::processInsert(const DbTable* table, typeDataObj dataObj, typeDba bdba, typeSlot slot, uint64_t offset) {
        ...
        switch (table->systemTable) {
            ...
            case DbTable::TABLE::SYS_CCOL:
                updateAllValues(&metadata->schema->sysCColPack, table, metadata->schema->sysCColPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_CDEF:
                updateAllValues(&metadata->schema->sysCDefPack, table, metadata->schema->sysCDefPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_COL:
                updateAllValues(&metadata->schema->sysColPack, table, metadata->schema->sysColPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_DEFERRED_STG:
                updateAllValues(&metadata->schema->sysDeferredStgPack, table, metadata->schema->sysDeferredStgPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_ECOL:
                updateAllValues(&metadata->schema->sysEColPack, table, metadata->schema->sysEColPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_LOB:
                updateAllValues(&metadata->schema->sysLobPack, table, metadata->schema->sysLobPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_LOB_COMP_PART:
                updateAllValues(&metadata->schema->sysLobCompPartPack, table, metadata->schema->sysLobCompPartPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_LOB_FRAG:
                updateAllValues(&metadata->schema->sysLobFragPack, table, metadata->schema->sysLobFragPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_OBJ:
                updateAllValues(&metadata->schema->sysObjPack, table, metadata->schema->sysObjPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_TAB:
                updateAllValues(&metadata->schema->sysTabPack, table, metadata->schema->sysTabPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_TABCOMPART:
                updateAllValues(&metadata->schema->sysTabComPartPack, table, metadata->schema->sysTabComPartPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_TABPART:
                updateAllValues(&metadata->schema->sysTabPartPack, table, metadata->schema->sysTabPartPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_TABSUBPART:
                updateAllValues(&metadata->schema->sysTabSubPartPack, table, metadata->schema->sysTabSubPartPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_TS:
                updateAllValues(&metadata->schema->sysTsPack, table, metadata->schema->sysTsPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::SYS_USER:
                updateAllValues(&metadata->schema->sysUserPack, table, metadata->schema->sysUserPack.forInsert(ctx, rowId, offset), offset);
                break;

            case DbTable::TABLE::XDB_TTSET:
                updateAllValues(&metadata->schema->xdbTtSetPack, table, metadata->schema->xdbTtSetPack.forInsert(ctx, rowId, offset), offset);
                break;
    ...
    void SystemTransaction::processDelete(const DbTable* table, typeDataObj dataObj, typeDba bdba, typeSlot slot, uint64_t offset) {
        ...
        switch (table->systemTable) {
            ...
            case DbTable::TABLE::SYS_CCOL:
                metadata->schema->sysCColPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_CDEF:
                metadata->schema->sysCDefPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_COL:
                metadata->schema->sysColPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_DEFERRED_STG:
                metadata->schema->sysDeferredStgPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_ECOL:
                metadata->schema->sysEColPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_LOB:
                metadata->schema->sysLobPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_LOB_COMP_PART:
                metadata->schema->sysLobCompPartPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_LOB_FRAG:
                metadata->schema->sysLobFragPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_OBJ:
                metadata->schema->sysObjPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_TAB:
                metadata->schema->sysTabPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_TABCOMPART:
                metadata->schema->sysTabComPartPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_TABPART:
                metadata->schema->sysTabPartPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_TABSUBPART:
                metadata->schema->sysTabSubPartPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_TS:
                metadata->schema->sysTsPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::SYS_USER:
                metadata->schema->sysUserPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::XDB_TTSET:
                metadata->schema->xdbTtSetPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::XDB_XNM:
                findMatchingXmlCtx(table)->xdbXNmPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::XDB_XPT:
                findMatchingXmlCtx(table)->xdbXPtPack.drop(ctx, rowId, offset, true);
                break;

            case DbTable::TABLE::XDB_XQN:
                findMatchingXmlCtx(table)->xdbXQnPack.drop(ctx, rowId, offset, true);
                break;
        }
    }
