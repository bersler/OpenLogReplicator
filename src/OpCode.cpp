...
namespace OpenLogReplicator {
    ...
    OpCode::OpCode(OracleAnalyser *oracleAnalyser, RedoLogRecord *redoLogRecord):
    ...
    void OpCode::process(void) {
        if (oracleAnalyser->dumpRedoLog >= 1) {
            ...
            if (oracleAnalyser->version < 0x12000) {
                if (redoLogRecord->typ == 6)
                    oracleAnalyser->dumpStream << "CHANGE #" << dec << (uint64_t)redoLogRecord->vectorNo <<
            ...
            } else if (oracleAnalyser->version < 0x12200) {
                if (redoLogRecord->typ == 6)
                    oracleAnalyser->dumpStream << "CHANGE #" << dec << (uint64_t)redoLogRecord->vectorNo <<
                        " MEDIA RECOVERY MARKER" <<
        ...
        if (oracleAnalyser->dumpRawData)
            redoLogRecord->dumpHex(oracleAnalyser->dumpStream, oracleAnalyser);
    }

    void OpCode::ktbRedo(uint64_t fieldPos, uint64_t fieldLength) {
        if (fieldLength < 8) {
            oracleAnalyser->dumpStream << "ERROR: too short field KTB Redo: " << dec << fieldLength << endl;
            return;
        }

        if (redoLogRecord->opc == 0x0A16)
            oracleAnalyser->dumpStream << "index undo for leaf key operations" << endl;
        else if (redoLogRecord->opc == 0x0B01)
            oracleAnalyser->dumpStream << "KDO undo record:" << endl;
        ...
        if (oracleAnalyser->dumpRedoLog >= 1) {
            ...
            oracleAnalyser->dumpStream << "KTB Redo " << endl;
            oracleAnalyser->dumpStream << "op: 0x" << setfill('0') << setw(2) << hex << (int32_t)op << " " <<
                    " ver: 0x" << setfill('0') << setw(2) << hex << (uint64_t)ver << "  " << endl;
            oracleAnalyser->dumpStream << "compat bit: " << dec << (uint64_t)(flg & 0x04) << " ";
            if ((flg & 0x04) != 0)
                oracleAnalyser->dumpStream << "(post-11)";
            else
                oracleAnalyser->dumpStream << "(pre-11)";
            oracleAnalyser->dumpStream << " padding: " << padding << endl;
        }
        char opCode = '?';

        if ((op & 0x0F) == KTBOP_C) {
            if (fieldLength < 16) {
                oracleAnalyser->dumpStream << "ERROR: too short field KTB Redo C: " << dec << fieldLength << endl;
                return;
            }
            ...
            if (oracleAnalyser->dumpRedoLog >= 1) {
                oracleAnalyser->dumpStream << "op: " << opCode << " " << " uba: " << PRINTUBA(redoLogRecord->uba) << endl;
            }
        } else if ((op & 0x0F) == KTBOP_Z) {
            opCode = 'Z';
            if (oracleAnalyser->dumpRedoLog >= 1) {
                oracleAnalyser->dumpStream << "op: " << opCode << endl;
            }
        } else if ((op & 0x0F) == KTBOP_L) {
            opCode = 'L';
            if ((flg & 0x08) == 0) {
                if (fieldLength < 28) {
                    oracleAnalyser->dumpStream << "ERROR: too short field KTB Redo L: " << dec << fieldLength << endl;
                    return;
                }
                redoLogRecord->uba = oracleAnalyser->read56(redoLogRecord->data + fieldPos + 12);

                if (oracleAnalyser->dumpRedoLog >= 1) {
                    typexid itlXid = XID(oracleAnalyser->read16(redoLogRecord->data + fieldPos + 4),
                            oracleAnalyser->read16(redoLogRecord->data + fieldPos + 6),
                            oracleAnalyser->read32(redoLogRecord->data + fieldPos + 8));

                    oracleAnalyser->dumpStream << "op: " << opCode << " " <<
                            " itl:" <<
                            " xid:  " << PRINTXID(itlXid) <<
                    ...
                    typescn scnx = oracleAnalyser->readSCNr(redoLogRecord->data + fieldPos + 26);
                ...
                redoLogRecord->uba = oracleAnalyser->read56(redoLogRecord->data + fieldPos + 16);

                if (oracleAnalyser->dumpRedoLog >= 1) {
                    typexid itlXid = XID(oracleAnalyser->read16(redoLogRecord->data + fieldPos + 8),
            ...
            if (fieldLength < 24) {
                oracleAnalyser->dumpStream << "ERROR: too short field KTB Redo F: " << dec << fieldLength << endl;
                return;
            }
        ...
        if ((op & KTBOP_BLOCKCLEANOUT) != 0) {
            if (oracleAnalyser->dumpRedoLog >= 1) {
                typescn scn = oracleAnalyser->readSCN(redoLogRecord->data + fieldPos + 48);
                ...
                if (oracleAnalyser->version < 0x12200)
                    oracleAnalyser->dumpStream << "Block cleanout record, scn: " <<
                            " " << PRINTSCN48(scn) <<
                ...
                if (fieldLength < 56 + entries * (uint64_t)8) {
                    oracleAnalyser->dumpStream << "ERROR: too short field KTB Redo F 0x11: " << dec << fieldLength << endl;
                    return;
                }
                ...
                for (uint64_t j = 0; j < entries; ++j) {
                    ...
                    typescn scn = oracleAnalyser->readSCNr(redoLogRecord->data + fieldPos + 58 + j * 8);
                    if (oracleAnalyser->version < 0x12100)
                        oracleAnalyser->dumpStream << "  itli: " << dec << (uint64_t)itli << " " <<
                                " flg: " << (uint64_t)flg << " " <<
    ...
    void OpCode::kdoOpCodeIRP(uint64_t fieldPos, uint64_t fieldLength) {
        if (fieldLength < 48) {
            oracleAnalyser->dumpStream << "ERROR: too short field KDO OpCode IRP: " << dec << fieldLength << endl;
            return;
        }

        redoLogRecord->fb = redoLogRecord->data[fieldPos + 16];
        redoLogRecord->cc = redoLogRecord->data[fieldPos + 18];
        redoLogRecord->slot = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 42);
        redoLogRecord->tabn = redoLogRecord->data[fieldPos + 44];

        if ((redoLogRecord->fb & FB_L) == 0) {
            redoLogRecord->nridBdba = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 28);
            redoLogRecord->nridSlot = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 32);
        }
        ...
        if (oracleAnalyser->dumpRedoLog >= 1) {
            uint16_t sizeDelt = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 40);
            ...
            oracleAnalyser->dumpStream << "fb: " << fbStr <<
                    " lb: 0x" << hex << (uint64_t)lb << " " <<
                    " cc: " << dec << (uint64_t)redoLogRecord->cc;
            if (fbStr[1] == 'C') {
                uint8_t cki = redoLogRecord->data[fieldPos + 19];
                oracleAnalyser->dumpStream << " cki: " << dec << (uint64_t)cki << endl;
            } else
                oracleAnalyser->dumpStream << endl;

            if ((redoLogRecord->fb & FB_F) != 0  && (redoLogRecord->fb & FB_H) == 0) {
                typedba hrid1 = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 20);
                typeslot hrid2 = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 24);
                oracleAnalyser->dumpStream << "hrid: 0x" << setfill('0') << setw(8) << hex << hrid1 << "." << hex << hrid2 << endl;
            }

            //next DBA/SLT
            if ((redoLogRecord->fb & FB_L) == 0) {
                oracleAnalyser->dumpStream << "nrid:  0x" << setfill('0') << setw(8) << hex << redoLogRecord->nridBdba << "." << hex << redoLogRecord->nridSlot << endl;
            }

            if ((redoLogRecord->fb & FB_K) != 0) {
                uint8_t curc = 0; //FIXME
                uint8_t comc = 0; //FIXME
                uint32_t pk = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 20);
                uint8_t pk1 = redoLogRecord->data[fieldPos + 24];
                uint32_t nk = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 28);
                uint8_t nk1 = redoLogRecord->data[fieldPos + 32];

                oracleAnalyser->dumpStream << "curc: " << dec << (uint64_t)curc <<
                        " comc: " << dec << (uint64_t)comc <<
                        " pk: 0x" << setfill('0') << setw(8) << hex << pk << "." << hex << (uint64_t)pk1 <<
                        " nk: 0x" << setfill('0') << setw(8) << hex << nk << "." << hex << (uint64_t)nk1 << endl;
            }

            oracleAnalyser->dumpStream << "null:";
            if (redoLogRecord->cc >= 11)
                oracleAnalyser->dumpStream << endl << "01234567890123456789012345678901234567890123456789012345678901234567890123456789" << endl;
            else
                oracleAnalyser->dumpStream << " ";
    ...
    void OpCode::kdoOpCodeDRP(uint64_t fieldPos, uint64_t fieldLength) {
        if (fieldLength < 20) {
            oracleAnalyser->dumpStream << "ERROR: too short field KDO OpCode DRP: " << dec << fieldLength << endl;
            return;
        }

        redoLogRecord->slot = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 16);
    ...
    void OpCode::kdoOpCodeLKR(uint64_t fieldPos, uint64_t fieldLength) {
        if (fieldLength < 20) {
            oracleAnalyser->dumpStream << "ERROR: too short field KDO OpCode LKR: " << dec << fieldLength << endl;
            return;
        }
    ...
    void OpCode::kdoOpCodeURP(uint64_t fieldPos, uint64_t fieldLength) {
        if (fieldLength < 28) {
            oracleAnalyser->dumpStream << "ERROR: too short field KDO OpCode URP: " << dec << fieldLength << endl;
            return;
        }

        redoLogRecord->fb = redoLogRecord->data[fieldPos + 16];
        redoLogRecord->slot = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 20);
        ...
        if (oracleAnalyser->dumpRedoLog >= 1) {
            ...
            int16_t size = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 24); //signed
    ...
    void OpCode::kdoOpCodeCFA(uint64_t fieldPos, uint64_t fieldLength) {
        if (fieldLength < 32) {
            oracleAnalyser->dumpStream << "ERROR: too short field KDO OpCode ORP: " << dec << fieldLength << endl;
            return;
        }

        redoLogRecord->nridBdba = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 16);
        redoLogRecord->nridSlot = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 20);
        redoLogRecord->slot = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 24);
    ...
    void OpCode::kdoOpCodeSKL(uint64_t fieldPos, uint64_t fieldLength) {
        if (fieldLength < 20) {
            oracleAnalyser->dumpStream << "ERROR: too short field KDO OpCode SKL: " << dec << fieldLength << endl;
            return;
        }
        ...
        if (oracleAnalyser->dumpRedoLog >= 1) {
            ...
            oracleAnalyser->dumpStream << "flag: " << flagStr <<
                    " lock: " << dec << (uint64_t)lock <<
                    " slot: " << dec << redoLogRecord->slot << "(0x" << hex << redoLogRecord->slot << ")" << endl;

            if ((flag & 0x01) != 0) {
                uint8_t fwd[4];
                uint16_t fwd2 = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 20);
                memcpy(fwd, redoLogRecord->data + fieldPos + 16, 4);
                oracleAnalyser->dumpStream << "fwd: 0x" <<
            ...
            if ((flag & 0x02) != 0) {
                uint8_t bkw[4];
                uint16_t bkw2 = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 26);
                memcpy(bkw, redoLogRecord->data + fieldPos + 22, 4);
                oracleAnalyser->dumpStream << "bkw: 0x" <<
    ...
    void OpCode::kdoOpCodeQM(uint64_t fieldPos, uint64_t fieldLength) {
        if (fieldLength < 24) {
            oracleAnalyser->dumpStream << "ERROR: too short field KDO OpCode QMI (1): " << dec << fieldLength << endl;
            return;
        }
    ...
    void OpCode::kdoOpCode(uint64_t fieldPos, uint64_t fieldLength) {
        if (fieldLength < 16) {
            oracleAnalyser->dumpStream << "ERROR: too short field KDO OpCode: " << dec << fieldLength << endl;
            return;
        }

        redoLogRecord->bdba = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 0);
        ...
        if (oracleAnalyser->dumpRedoLog >= 1) {
            typedba hdba = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 4);
            uint16_t maxFr = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 8);
            ...
            switch (redoLogRecord->op & 0x1F) {
                ...
                default:
                    opCode = "XXX";
                    if (oracleAnalyser->dumpRedoLog >= 1)
                        oracleAnalyser->dumpStream << "DEBUG op: " << dec << (uint64_t)(redoLogRecord->op & 0x1F) << endl;
            }
            ...
            oracleAnalyser->dumpStream << "KDO Op code: " << opCode << " row dependencies " << rowDependencies << endl;
            oracleAnalyser->dumpStream << "  xtype: " << xtype << rtype <<
            ...
            oracleAnalyser->dumpStream << "itli: " << dec << (uint64_t)redoLogRecord->itli << " " <<
    ...
    void OpCode::ktub(uint64_t fieldPos, uint64_t fieldLength) {
        ...
        redoLogRecord->tsn = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 8);
        redoLogRecord->undo = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 12);
        ...
        redoLogRecord->flg = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 20);
        ...
        if (oracleAnalyser->version < 0x19000) {
            ...
            typedba prevDba = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 12);
            uint16_t wrp = oracleAnalyser->read16(redoLogRecord->data + fieldPos + 22);
        ...
        } else if (oracleAnalyser->version < 0x19000) {
            if ((redoLogRecord->flg & FLG_MULTIBLOCKUNDOHEAD) != 0)
                undoType = "Multi-Block undo - HEAD";
            else if ((redoLogRecord->flg & FLG_MULTIBLOCKUNDOTAIL) != 0)
                undoType = "Multi-Block undo - TAIL";
        ...
        if (isKtubl) {
            //KTUBL
            if (fieldLength < 28) {
                oracleAnalyser->dumpStream << "too short field ktubl: " << dec << fieldLength << endl;
                return;
            }
            ...
            if (fieldLength == 28) {
                ...
                if (oracleAnalyser->dumpRedoLog >= 1) {
                    ...
                    uint32_t prevBrb = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 64);
                    uint32_t prevBcl = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 68);
                    uint32_t logonUser = oracleAnalyser->read32(redoLogRecord->data + fieldPos + 72);
    ...
    void OpCode::dumpColsVector(uint8_t *data, uint64_t colnum, uint16_t fieldLength) {
        ...
        oracleAnalyser->dumpStream << "Vector content: " << endl;
        ...
        for (uint64_t k = 0; k < redoLogRecord->cc; ++k) {
            ...
            if (fieldLength == 0xFE) {
                fieldLength = oracleAnalyser->read16(data + pos);
                pos += 2;
            }
    ...
    void OpCode::dumpCols(uint8_t *data, uint64_t colnum, uint16_t fieldLength, uint8_t isNull) {
        if (isNull) {
            oracleAnalyser->dumpStream << "col " << setfill(' ') << setw(2) << dec << colnum << ": *NULL*" << endl;
        } else {
            oracleAnalyser->dumpStream << "col " << setfill(' ') << setw(2) << dec << colnum << ": " <<
            ...
            for (uint64_t j = 0; j < fieldLength; ++j) {
                oracleAnalyser->dumpStream << " " << setfill('0') << setw(2) << hex << (uint64_t)data[j];
    ...
    void OpCode::dumpRows(uint8_t *data) {
        if (oracleAnalyser->dumpRedoLog >= 1) {
            ...
            for (uint64_t r = 0; r < redoLogRecord->nrow; ++r) {
                oracleAnalyser->dumpStream << "slot[" << dec << r << "]: " << dec << oracleAnalyser->read16(redoLogRecord->data + redoLogRecord->slotsDelta + r * 2) << endl;
                processFbFlags(data[pos + 0], fbStr);
                uint8_t lb = data[pos + 1];
                uint8_t jcc = data[pos + 2];
                uint16_t tl = oracleAnalyser->read16(redoLogRecord->data + redoLogRecord->rowLenghsDelta + r * 2);

                oracleAnalyser->dumpStream << "tl: " << dec << tl <<
                        " fb: " << fbStr <<
                        " lb: 0x" << hex << (uint64_t)lb << " " <<
