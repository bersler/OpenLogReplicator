    ...
    int OpenLogReplicator::run() {
        ...
        for (rapidjson::SizeType j = 0; j < sourceArrayJson.Size(); ++j) {
            ...
            if (sourceJson.HasMember("memory")) {
                ...
                if (!ctx->disableChecksSet(Ctx::DISABLE_CHECKS_JSON_TAGS)) {
                    static const char* memoryNames[] = {"min-mb", "max-mb", "reserve-mb", "swap-mb", "swap-path", "read-buffer-max-mb", nullptr};
                ...
                if (memoryJson.HasMember("reserve-mb")) {
                    memoryReserveMb = Ctx::getJsonFieldU64(configFileName, memoryJson, "resere-mb");
                    if (memoryReserveMb > memoryMaxMb - 4)
                        throw ConfigurationException(30001, "bad JSON, invalid \"reserve-mb\" value: " + std::to_string(memoryReserveMb) +
            ...
            // MEMORY MANAGER
            ctx->initialize(memoryMinMb, memoryMaxMb, memoryReserveMb, memorySwapMb, readBufferMax);
