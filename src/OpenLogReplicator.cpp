        ...
        for (rapidjson::SizeType j = 0; j < sourceArrayJson.Size(); ++j) {
            ...
            if (!ctx->isDisableChecksSet(Ctx::DISABLE_CHECKS::JSON_TAGS)) {
                static const char* sourceNames[]{"alias", "memory", "name", "reader", "flags", "state", "debug",
                                                 ...
                                                 "arch-read-tries", "redo-verify-delay-us", "refresh-interval-us", "arch", "key",
                                                 "filter", nullptr};
            ...
            if (sourceJson.HasMember("key") && !ctx->isFlagSet(Ctx::REDO_FLAGS::SCHEMALESS)) {
                const rapidjson::Value& keyJson = Ctx::getJsonFieldO(configFileName, sourceJson, "key");

                if (!ctx->isDisableChecksSet(Ctx::DISABLE_CHECKS::JSON_TAGS)) {
                    static const char* keyNames[]{"separator", "table", nullptr};
                    Ctx::checkJsonFields(configFileName, keyJson, keyNames);
                }

                if (keyJson.HasMember("separator"))
                    metadata->keySeparator = Ctx::getJsonFieldS(configFileName, Ctx::JSON_SEPARATOR_LENGTH, keyJson, "separator");

                if (keyJson.HasMember("table")) {
                    const rapidjson::Value& tableArrayJson = Ctx::getJsonFieldA(configFileName, keyJson, "table");

                    for (rapidjson::SizeType k = 0; k < tableArrayJson.Size(); ++k) {
                        const rapidjson::Value& tableElementJson = Ctx::getJsonFieldO(configFileName, tableArrayJson, "table", k);

                        if (!ctx->isDisableChecksSet(Ctx::DISABLE_CHECKS::JSON_TAGS)) {
                            static const char* tableElementNames[]{"owner", "table", "columns", nullptr};
                            Ctx::checkJsonFields(configFileName, tableElementJson, tableElementNames);
                        }

                        const char* owner = Ctx::getJsonFieldS(configFileName, SysUser::NAME_LENGTH, tableElementJson, "owner");
                        const char* table = Ctx::getJsonFieldS(configFileName, SysObj::NAME_LENGTH, tableElementJson, "table");
                        const char* columns = Ctx::getJsonFieldS(configFileName, Ctx::JSON_COLUMNS_LENGTH, tableElementJson, "columns");
                        SchemaKey* key = new SchemaKey(owner, table, columns);

                        metadata->keyElements.push_back(key);
                    }
                }
                // table
            }
