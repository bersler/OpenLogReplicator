...
Executing query to read current SCN from database failed. Verify if the database is available.

==== code 10030: "getting database sequence for scn: <scn"

Executing query to read database sequence for provided SCN failed. Verify if the database is available (TODO: ref.).
...
Parsing configuration file failed. Check if the file is not corrupted. Please verify content of JSON file using online JSON validator.
...
Parsing of a JSON file failed. File contains invalid data. Verify if file content is a valid JSON.
