...
|`read-buffer-min-mb`
|_integer_, min: 4, max: min(`max-mb` - `unswap-min-mb` - `write-buffer-min-mb` - 4, `read-buffer-max-mb`), default: 4
|Size of memory buffer reserved for reading new redo log data.

|`swap-mb`
|_integer_, min: 0, max: `max-mb`, default: `max-mb * 3 / 4`
...
|_integer_, min: 4, max: `max-mb` - `write-buffer-min-mb` - `read-buffer-min-mb` - 4, default: 4 (when swapping is enabled)
