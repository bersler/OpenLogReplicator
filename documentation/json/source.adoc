...
|`format`
|_element_ — see xref:format.adoc#format[format], mandatory
...
|`reader`
|_element_ — see xref:reader.adoc#reader[reader], mandatory
...
|`debug`
|_element_ — see xref:debug.adoc#debug[debug]
|Group of debugging options.

|`filter`
|_element_ — see xref:filter.adoc#filter[filter]
...
|`metrics`
|_element_ — see xref:metrics.adoc#metrics[metrics]
|Options to collect and export application metrics.
...
* `0x0001` — Read-only archived logs (do not read online redo).
...
_NOTE:_ Refer to the xref:../user-manual/user-manual.adoc[User Manual] for details on schemaless and adaptive schema modes.

|`memory`
|_element_ — see xref:memory.adoc#memory[memory]
