...
_CAUTION:_ `protobuf` support is experimental.
...
|Encoding/format for character types (`CHAR`, `VARCHAR2`, `CLOB`).
...
* `0x0001` — include `scn` in every message (default is first message only).
* `0x0002` — copy SCNs from the commit record for all DML operations.
...
|_integer_, min: 0, max: 1, default: 0
|When `1`, include the chosen `timestamp` field in every payload; `0` adds it to the first message only.
...
* Test `protobuf` output carefully before deploying it to consumers that expect strict schema evolution.
