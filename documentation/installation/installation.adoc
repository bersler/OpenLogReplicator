...
* Supported versions: 11.2, 12.1, 12.2, 18c, 19c, 21c, 23c, 23ai, 26ai.

* Supported editions: XE/FREE/AI, DE, SE, SE2, PE, EE.
...
[NOTE]
====
Minimal supplemental logging is required for OpenLogReplicator to work.
...
[CAUTION]
====
OpenLogReplicator rigorously tests the correctness of the custom key columns.
...
[CAUTION]
====
Adding all columns to supplemental logging can cause a significant increase of redo log size.
...
[IMPORTANT]
====
It is not recommended to run OpenLogReplicator using SYS or SYSTEM account or any account with high privileges to connect to the database.
...
[NOTE]
====
Grant script has to be run from _SYS_ account.
