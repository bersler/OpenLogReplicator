...
Please refer to a full parameters list for more details.
...
_NOTE:_ Please refer to details in xref:../user-manual/user-manual.adoc#schemaless-mode[the User Manual] for details.

* `0x0004` -– Adaptive schema mode.
This mode is only valid when schemaless mode has been chosen.

_NOTE:_ Please refer to details in xref:../user-manual/user-manual.adoc#adaptive-schema-mode_[the User Manual] for details.
...
|`scn` [[scn]]
|_number_, min: 0, max: 3, default: 0
|By default, SCN is present only in a first message (when message = `0`) in scn field.

Number is a sum of:

* `0x0001` -– Print SCN values in hexadecimal format (in _“C”_ format – like `0xFF`) in `scns` field.
Otherwise, the SCN value is stored as a decimal number in `scn` field.

* `0x0002` -- Put SCN field in every message (when a message is set to `0`).
If this parameter is not used, the `scn`/`scns` field is present just in checkpoint records and begin of transaction records.

|`timestamp` [[timestamp]]
|_number_, min: 0, max: 3, default: 0
|Format of timestamp values.
By default `tm` field is used and timestamp in Unix Epoch format (example: `"tm": 1679100920000`).

Number is a sum of:

* `0x0001` -- Use _tms_ field name instead and assign string value of timestamp in _ISO-8601_ format (`"tms":"YYYY-MM-DDTHH:MI:SSS"`).

* `0x0002` -- Put timestamp field in every message (when _message_ field doesn't contain the bit set to `0x0001`).
...
|`enable-idempotence`
|_number_, min: 0, max: 1, default: 1
|Idempotent producer is enabled when parameter set to `1`.
...
Please refer to the documentation of your C library for more information.
