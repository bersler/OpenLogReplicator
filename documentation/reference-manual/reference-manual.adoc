...
|`key`
|_element_ of <<key,key>>
|Configuration for the metadata added to the target message.
Depending on the type of the writer, this can be additional key metadata for Kafka or additional metadata for Debezium (in the future).
...
|Debugging parameter which allows copying all contents of processed redo log files to defined folder.
...
The columns are separated by a comma character.
...
|`table`
|_list_ of a <<table-kafka,table>> element
|List of table regex rules which define the key for Kafka.

The list of rules is traversed for every table.
If a match appears, the key is set for the table and further matching is not performed.
Example:
`{"owner": "OWNER1", "table": "K.\*", "columns": "[ui]"},
{"owner": "OWNER1", "table": "L.\*", "columns": "[pk]"},
{"owner": "OWNER1", "table": "M.\*", "columns": "a,b,c"},
{"owner": "OWNER1", "table": ".\*", "columns": "[all]"}`

In the following example if the table matches rule `OWNER.K.*` then the unique index columns are used as a key.
If that rules is not matched, the next rule is checked, etc.
The last rule matches every table owned by OWNER1 and uses all columns as a key.

|`separator`
|_string_, max length: 128, default: `","`
|Separator string for key fields which are concatenated for Kafka using `RD_KAFKA_VTYPE_KEY` librdkafka message parameter during the send operation.
...
[[table-kafka]]
[width="100%",cols="a,a,50%a",options="header"]
.Table element for a key element.
|===
...
|A string field with a list of columns which should be used as a key.

Instead of column list, the following special values can be used:

* `[pk]` -- Use primary key columns.

* `[ui]` -- Use unique index columns.
