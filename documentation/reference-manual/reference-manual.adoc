...
* `0x0080` -- Show nested columns in output.

* `0x0100` -- Show unused columns in output.

* `0x0200` -- Include incomplete transactions in output.
...
* `0x0400` -- Include system transactions in output.

* `0x0800` -- Show checkpoint information in output.
...
* `0x1000` -- Don’t delete old checkpoint files.
...
* `0x4000` -- Send column data to output in raw (hex) format.
...
* `0x0000` -- SCN is stored as a decimal number in `scn` field.

* `0x0001` -– SCN values is stored in a text format in hexadecimal format (in _“C”_ format – like `0xFF`) in `scns` field.
...
* `0x0000` -- Put `scn` field only in the first message.
...
* `0x0000` -- Unix with nanoseconds -- `"tm": 1651384800123456789`.
* `0x0001` -- Unix with microseconds precision -- `"tm": 1651384800123457`.
* `0x0002` -- Unix with milliseconds precision -- `"tm": 1651384800123`.
* `0x0003` -- Unix with seconds precision -- `"tm": 1651384800`.
* `0x0004` -- Unix with nanoseconds stored as character -- `"tms": "1651384800123456789"`.
* `0x0005` -- Unix with microseconds precision stored as character -- `"tms": "1651384800123457"`.
* `0x0006` -- Unix with milliseconds precision stored as character -- `"tms": "1651384800123"`.
* `0x0007` -- Unix with seconds precision stored as character -- `"tms": "1651384800"`.
* `0x0008` -- _ISO-8601_ format stored as character -- `"tms": "2022-05-01 06:00:00.123456789"`.

|`timestamp-tz` [[timestamp]]
|_number_, min: 0, max: 4, default: 0
|Format of timestamp with timezone values.

In the following description, the following timerstamp with timezone is used as an example: `"2022-05-01 06:00:00.123456789 Europe/Warsaw"`.
Possible values are:

* `0x0000` -- Unix with nanoseconds stored as character with timezone after comma sign -- `"tms": "1651384800123456789,Europe/Warsaw"`.
* `0x0001` -- Unix with microseconds precision stored as character with timezone after comma sign -- `"tms": "1651384800123457,Europe/Warsaw"`.
* `0x0002` -- Unix with milliseconds precision stored as character with timezone after comma sign -- `"tms": "1651384800123,Europe/Warsaw"`.
* `0x0003` -- Unix with seconds precision stored as character with timezone after comma sign -- `"tms": "1651384800,Europe/Warsaw"`.
* `0x0004` -- _ISO-8601_ format stored as character with timezone after space sign-- `"tms": "2022-05-01 06:00:00.123456789 Europe/Warsaw"`.
...
* `0x0000` -- Put `timestamp` field only in the first message.

* `0x0001` -- Put `timestamp` field in every message.
...
When using `%i` or `%t` format `max-size` parameter must be set to value greater than 0.
