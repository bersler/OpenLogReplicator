...
- SCN checkpoint -- the position in the redo log is defined by the SCN number;
...
CAUTION: By default rhe schema checkpoints are deleted when they are not needed anymore. Different checkpoint options decide how many files are created and how many files are kept. It is also possible to disable the deletion of the schema checkpoints.
