...
CAUTION: Currently also LOB data is stored in dynamic memory, but is planned to be moved to the main memory buffers.
...
NOTE: Depending on user configuration, the output stream in the picture above might contain 2 or 6 messages.
...
For cases where there is no physical way to connect the database, OpenLogReplicator offers
